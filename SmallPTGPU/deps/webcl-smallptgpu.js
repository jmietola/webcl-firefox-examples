...
function setupWebCL() {
	...
	try {
		...
		cl = WebCL.createContext({platform: selectedPlatform, devices: selectedDevice, deviceType: WebCL.CL_DEVICE_TYPE_GPU});
	...
	try {
		...
		clProgram.build([selectedDevice],opts);		
		var buildLog = clProgram.getBuildInfo(selectedDevice, WebCL.CL_PROGRAM_BUILD_LOG);
	...
	workGroupSizeSmallPT = clKernelsSmallPT.getWorkGroupInfo(selectedDevice, WebCL.CL_KERNEL_WORK_GROUP_SIZE);

	clKernelsToneMapping = clProgram.createKernel("WebCLToneMapping");
	workGroupSizeToneMapping = clKernelsToneMapping.getWorkGroupInfo(selectedDevice, WebCL.CL_KERNEL_WORK_GROUP_SIZE);
...
function executeSmallPTKernel() {
	...
	clKernelsSmallPT.setArg(0, clColorBuffer);
	clKernelsSmallPT.setArg(1, clSeedBuffer);
	clKernelsSmallPT.setArg(2, clCameraBuffer);
	clKernelsSmallPT.setArg(3,  new Uint32Array([scene.spheres.length]));
	clKernelsSmallPT.setArg(4, clSphereBuffer);
	clKernelsSmallPT.setArg(5,  new Uint32Array([canvas.width]));
	clKernelsSmallPT.setArg(6,  new Uint32Array([canvas.height]));
	clKernelsSmallPT.setArg(7,  new Uint32Array([currentSample++]));
...
function executeToneMappingKernel() {
	...
	clKernelsToneMapping.setArg(0, clColorBuffer);
	clKernelsToneMapping.setArg(1, clPixelsBuffer);
	clKernelsToneMapping.setArg(2, new Uint32Array([canvas.width]));
	clKernelsToneMapping.setArg(3, new Uint32Array([canvas.height]));
